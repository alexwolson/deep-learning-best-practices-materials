...
A simple script to preprocess the CUB-200-2011 dataset for PyTorch/Lightning.
It:
  1. Reads the official data splits from train_test_split.txt (1 = train, 0 = test).
  2. Associates each image ID with:
       - The image file path (from images.txt)
       - The bounding box (from bounding_boxes.txt)
       - The class label (from image_class_labels.txt)
       - The class name (from classes.txt)
  3. Resizes each image to 224x224, scaling the bounding box accordingly.
  4. Creates two CSV files: train.csv and val.csv, each containing:
       filename, class_id, class_name, x_min, y_min, x_max, y_max
  5. Saves resized images in data/processed_cub/images/

Usage:
  python preprocess_cub.py --cub_dir data/raw/CUB_200_2011 --out_dir data/processed_cub
"""
...
def load_dict_from_txt(txt_path, has_index=True):
    """
    Loads a file (like images.txt, bounding_boxes.txt, etc.) where each line is:
      ID <something...>
    Returns a dict mapping ID -> everything else on that line.

    If has_index=False, we just read the line as it is (for classes).
    """
    output = {}
...
def resize_image_and_bbox(in_path, out_path, bbox, target_size=(224, 224)):
    """
    Resizes the image at in_path to `target_size`, scales the bounding box accordingly,
    and saves the resized image to out_path. Returns the scaled bbox (x_min, y_min, x_max, y_max).
    """
